...
export const CheckoutSale = () => { 
  ...
  useEffect(() => {
    document.title = "Comprar Imóvel - Hub Temporada";
  }, []);

  if (!destination || !destination.valor_venda) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-6 bg-paper">
        <h2 className="text-2xl font-serif">Nenhum imóvel selecionado para compra</h2>
  ...
  const valorTotal = destination.valor_venda;
  const comissao = valorTotal * 0.20;
  ...
  const handlePayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('/api/create-sale-session', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          totalAmount: comissao,
  ...
  return (
    <div className="min-h-screen bg-paper py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto">
        ...
        <h1 className="text-3xl font-serif mb-8 text-center">Comprar Imóvel</h1>
        ...
        <div className="bg-white p-8 rounded-xl shadow-sm border border-ink/10 mb-8">
          <div className="flex flex-col md:flex-row gap-6 items-center border-b border-ink/10 pb-6 mb-6">
            ...
            <div className="text-center md:text-right">
              <p className="text-sm opacity-60">Sinal (20%)</p>
              <p className="text-3xl font-serif text-green-600">R$ {comissao.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
            </div>
          </div>
          ...
          <form onSubmit={handlePayment} className="space-y-6">
            ...
            <div>
              <h3 className="text-sm uppercase tracking-widest opacity-50 mb-4">Forma de Pagamento (Sinal)</h3>
            ...
            <button 
              type="submit"
              disabled={loading}
              className="w-full py-4 bg-green-600 text-white rounded-lg text-sm uppercase tracking-widest hover:bg-green-700 transition-all disabled:opacity-50 font-medium mt-4"
            >
              {loading ? 'Processando...' : `Pagar Sinal de R$ ${comissao.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`}
            </button>
          </form>
        </div>
      </div>
